...
def user(uid):
    ...
    if 'CAS_USERNAME' in session:
        ...
        return render_template("user.html", page_title = user['display_name'],
            user = user, friendsList = friendsList, playlists = playlists)
    else:
...
def createPlaylist():
    ...
    if 'CAS_USERNAME' in session:
        ...
        if request.method == 'GET':
            ...
            if playlist.check_unique_playlist_name(conn, pName, username):
                playlist.createPlaylist(conn,pName,pGenre, username)
